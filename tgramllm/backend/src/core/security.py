"""
tgrambuddy/tgramllm/src/llm/core/security.py
...
async def fetch_user_by_email(email: str, db_adapter: DBAdapter) -> Optional[User]:
    ...
    try:
        user: Optional[User]  = await session.scalar(select(User).where(User.email == email))
        logging.info('----------->Found user is: %s', user)
        return user

    except SQLAlchemyError as e:
        await session.rollback()
        logging.error("Error while fetching  user:: %s from database.",e, exc_info=True)
        return None
...
async def fetch_user_by_jwt(
    ...
    try:
        ...
        user: Optional[User]  = await session.scalar(select(User).where(User.email == token_data.email))
...
async def authenticate_user(
    ...
    """
    user: Optional[User] = await session.scalar(select(User).where(User.email == email_to_auth))
